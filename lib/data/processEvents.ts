...
export const INEFFICIENCIES: Inefficiency[] = [
  {
    ...
    impact: "Delayed batch disposition decisions; audit exposure",
    severity: "high",
    case_count: PROCESS_CASES.filter(c => c.events.some(e => e.violation_type?.includes("Late containment"))).length,
    recommendation: "Implement electronic containment forms with mandatory same-day timestamp validation",
  },
  {
    id: "investigation-queue",
    title: "Investigation Queue Bottleneck",
    detail: "Average 6.2 days between triage completion and investigation start. SOP target: ≤3 days.",
    impact: "Extends total cycle time by ~3 days per case; ~180 days aggregate across 60 cases",
    severity: "high",
    case_count: PROCESS_CASES.filter(c => c.events.some(e => e.violation_type?.includes("queue wait"))).length,
